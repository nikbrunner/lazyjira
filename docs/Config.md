...
A status message confirms each copy. Remap the actions with `keybinding.issues.copyBranchName`, `createBranch`, `copyWorktreeName`, and `createWorktree`. Custom commands bound to the same key take precedence.

### Create a worktree

Press `W` in the issues, info, or detail panel. The string generated by `w` supplies both the branch name and the default directory name. Confirm or edit the destination under `worktree.defaultPath`; editing the destination changes only its path. Escape cancels without creating anything.
...
`defaultPath` is the parent directory for worktrees. It defaults to `".."`, placing them beside the main checkout. Relative paths resolve from the main checkout even when lazyjira runs from a linked worktree or subdirectory. Absolute paths and `~/…` are supported. The destination prompt also accepts relative paths using that same base.

New branches start at the current checkout's HEAD. Existing local branches are reused. Branches checked out in another worktree and existing destination paths report an error. Existing files and branches are never force-overwritten.

After creation, lazyjira's Git context and custom commands use the new worktree. The original checkout and parent shell directory stay unchanged. The switch respects `git.closeOnCheckout`.
